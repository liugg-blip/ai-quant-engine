...
(async () => {
  ...
  // 回退：某分类只要有一个子查询失败，就整体沿用旧快照。
  // 例如「指数」包含沪市和深市两个查询，只保留成功的一半会制造一个看似新鲜但残缺的列表。
  ...
  const staleGroups = Array.from(new Set(failed));
  const out = {
    // ts 表示所有分类都不晚于的保守数据日期；不能把回退旧快照的打包时间冒充刷新日期。
    ts: staleGroups.length && old && old.ts ? old.ts : today,
    refreshed_at: today,
