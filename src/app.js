...
var I18N = window.QE_I18N || { lang: 'zh-CN', isEnglish: false, t: function (s) { return s; } };
var UI_LANG = I18N.lang || 'zh-CN';
var uiText = function (zh, en) { return I18N.isEnglish ? en : zh; };
...
function logBar(sym, b, prevC) {
  ...
  if (sym !== logSym) {
    logSym = sym;
    log('──── <span class="sym">' + esc(sym) + '</span> ' +
      (otc ? uiText('单位净值数据流', 'NAV data stream') : uiText('开高低收数据流', 'OHLC data stream')) + ' ────', 'sys');
  }
  var chg = prevC ? (b.c - prevC) / prevC * 100 : 0;
  var cl = chg >= 0 ? 'up' : 'dn';
  var body = otc
    ? uiText('净值 ', 'NAV ') + px(b.c)
    : uiText('开', 'O ') + px(b.o) + uiText(' 高', ' H ') + px(b.h) +
      uiText(' 低', ' L ') + px(b.l) + uiText(' 收', ' C ') + px(b.c);
...
/* ============================================================
   ...
   ============================================================ */
var LSKEY = 'qe_universe_v3';   // v3 记录滞后分类，弃用曾把回退时间误标成刷新时间的旧缓存
var DB = { ts: '', refreshedAt: '', staleGroups: [], list: [], byId: {}, byCat: {} };
...
function loadUniverse(u) {
  DB.ts = u.ts; DB.refreshedAt = u.refreshed_at || u.ts;
  DB.staleGroups = Array.isArray(u.stale_groups) ? u.stale_groups.slice() : [];
  DB.list = []; DB.byId = {}; DB.byCat = {};
...
function paintDbState() {
  $('dbTxt').textContent = I18N.isEnglish
    ? 'Universe ' + DB.list.length + ' · data date ' + (DB.ts || '—') +
      (DB.staleGroups.length ? ' · ' + DB.staleGroups.length + ' categories pending refresh' : '')
    : '标的库 ' + DB.list.length + ' 个 · 数据日期 ' + (DB.ts || '—') +
      (DB.staleGroups.length ? ' · ' + DB.staleGroups.length + ' 类待刷新' : '');
  $('dbTxt').title = CATS.map(function (c) {
    return (I18N.t ? I18N.t(c) : c) + ' ' + (DB.byCat[c] || []).length;
  }).join(' · ') + (DB.staleGroups.length
    ? (I18N.isEnglish ? '\nUsing previous snapshots: ' + DB.staleGroups.map(I18N.t).join(', ') + '; last attempt ' + DB.refreshedAt
      : '\n沿用旧快照：' + DB.staleGroups.join('、') + '；最近尝试 ' + DB.refreshedAt)
    : '');
...
function refreshDB() {
  ...
  var bag = {}, done = 0, totalPages = 0, staleGroups = [];
  ...
  chain.then(function () {
    ...
    var data = {}, total = 0;
    var required = ['股票', '基金', '行业板块', '概念板块', '地域板块', '指数', '美股', '场外基金'];
    var missing = required.filter(function (k) { return !bag[k] || !bag[k].length; });
    if (missing.length) throw new Error('以下分类未完整返回：' + missing.join('、'));
    ...
    var u = { ts: staleGroups.length ? DB.ts : today, refreshed_at: today,
      stale_groups: staleGroups, total: total, data: data };
    try { localStorage.setItem(LSKEY, JSON.stringify(u)); } catch (e) { log('本地存储写入失败（不影响本次使用）：' + e.message, 'err'); }
    loadUniverse(u);
    log((staleGroups.length ? '◐ 标的库已部分更新' : '✓ 标的库已更新') + '：共 ' + total + ' 个标的（' +
      Object.keys(data).map(function (k) { return k + ' ' + data[k].split('\n').length; }).join('，') + '）' +
      (staleGroups.length ? '；沿用旧快照：' + staleGroups.join('、') : ''), staleGroups.length ? 'sys' : 'ok');
    lbDone();
  }).catch(function (e) {
...
var X = null;   // 指标
var DATA_OP = { seq: 0, controller: null, streamTimer: 0, busy: false };
function setDataBusy(on) {
  DATA_OP.busy = on;
  ['btnFetch', 'btnPlay', 'btnProj', 'btnAI', 'btnTen', 'btnShort', 'btnRev', 'btnFactor', 'btnAlloc']
    .forEach(function (id) { if ($(id)) $(id).disabled = on; });
}

function jsonp(url, timeout, signal) {
  return new Promise(function (res, rej) {
    ...
    function done(fn, value) { if (settled) return; settled = true; clean(); fn(value); }
    function abort() { done(rej, new DOMException('请求已取消', 'AbortError')); }
    if (signal && signal.aborted) { abort(); return; }
    if (signal) signal.addEventListener('abort', abort, { once: true });
    window[cb] = function (d) { done(res, d); };
    s.onerror = function () { done(rej, new Error('网络不可达')); };
...
function fetchEM(secid, name, signal) {
  var adj = adjMode();
  var main = jsonp(klineUrl(secid, adj), 9000, signal).then(function (r) {
  ...
  var raw = jsonp(klineUrl(secid, '0'), 9000, signal).then(function (r) {
    var m = {};
...
function loadScript(url, timeout, signal) {
  return new Promise(function (res, rej) {
    var s = document.createElement('script'), settled = false;
    var tid = setTimeout(function () { clean(); rej(new Error('请求超时')); }, timeout || 15000);
    function clean() { clearTimeout(tid); if (signal) signal.removeEventListener('abort', abort); if (s.parentNode) s.parentNode.removeChild(s); }
    function done(fn, value) { if (settled) return; settled = true; clean(); fn(value); }
    function abort() { done(rej, new DOMException('请求已取消', 'AbortError')); }
    if (signal && signal.aborted) { abort(); return; }
    if (signal) signal.addEventListener('abort', abort, { once: true });
    s.onload = function () { done(res); };
    s.onerror = function () { done(rej, new Error('网络不可达')); };
    s.src = url;
    document.head.appendChild(s);
  });
...
function fetchOTCNav(code, name, fullHistory, signal) {
  ...
  return loadScript('https://fund.eastmoney.com/pingzhongdata/' + code + '.js?v=' + Date.now(), 20000, signal)
    .then(function () {
...
function mkt(d) {
  if (isUS(d)) return { lot: 1, stamp: 0, minComm: US_MIN_COMM, cur: '$', unit: '美元', lotName: '股' };
  var kind = (d && (d.cat || d.kind)) || '';
  var noStamp = !!(d && d.otc) || /基金|ETF|指数|板块/i.test(kind);
  return { lot: LOT, stamp: noStamp ? 0 : STAMP, minComm: MIN_COMM, cur: '¥', unit: '元', lotName: '手(100股)' };
}
...
function backtest(st, d, x, cfg, rng) {
  ...
  for (i = start; i < END; i++) {
    if (pos) {
      ...
      if (why) {
        ...
        // 委托在 i+1 的开盘价成交，权益也必须记到 i+1，不能倒签到信号日 i。
        pos = null; eq[i + 1] = cap;
      }
    } else {
      eq[i] = cap;
  ...
  eq[END] = cap;
  if (eq[END] > peak) peak = eq[END];
  var finalDd = (peak - eq[END]) / peak; if (finalDd > mdd) mdd = finalDd;
...
function buildPanel(list) {
  ...
  var syms = list.map(function (d) {
    var c = new Array(dates.length), o = new Array(dates.length), rc = new Array(dates.length), ro = new Array(dates.length);
    for (i = 0; i < d.t.length; i++) {
      var k = pos[d.t[i]];
      if (k !== undefined) { c[k] = d.c[i]; o[k] = d.o[i]; rc[k] = (d.rc || d.c)[i]; ro[k] = (d.ro || d.o)[i]; }
    }
    return { name: d.name, sym: d.sym, cat: d.cat, otc: !!d.otc, c: c, o: o, rc: rc, ro: ro, d: d };
  });
...
/* 从一笔现金中解算可买数量。返回的 units 按复权序列记账，价差/费用按不复权成交价扣除。 */
function allocationBuyOrder(d, budget, rawPrice, adjPrice, cfg) {
  if (!(budget > 0 && rawPrice > 0 && adjPrice > 0)) return null;
  var MK = mkt(d), otc = !!d.otc, slip = otc ? 0 : cfg.slip / 1e4;
  if (otc) {
    var rate = cfg.buyFee / 100, gross = budget / (1 + rate);
    return { units: gross / adjPrice, spent: gross * (1 + rate), fee: gross * rate, shares: gross / rawPrice };
  }
  var execPx = rawPrice * (1 + slip), comm = cfg.fee / 1e4;
  var cap = Math.min((budget - MK.minComm) / execPx, budget / (execPx * (1 + comm)));
  var shares = Math.floor(cap / MK.lot) * MK.lot;
  for (var g = 0; g < 3 && shares > 0; g++) {
    var execGross = shares * execPx, fee = Math.max(execGross * comm, MK.minComm);
    if (execGross + fee <= budget + 1e-8) {
      return { units: shares * rawPrice / adjPrice, spent: execGross + fee, fee: fee,
        shares: shares, slippage: shares * rawPrice * slip };
    }
    shares -= MK.lot;
  }
  return null;
}
function allocationSellOrder(d, unitsWanted, rawPrice, adjPrice, cfg) {
  if (!(unitsWanted > 0 && rawPrice > 0 && adjPrice > 0)) return null;
  var MK = mkt(d), otc = !!d.otc, slip = otc ? 0 : cfg.slip / 1e4;
  var gross, units;
  if (otc) { units = unitsWanted; gross = units * adjPrice; }
  else {
    var equivalentShares = unitsWanted * adjPrice / rawPrice;
    var shares = Math.floor(equivalentShares / MK.lot) * MK.lot;
    if (shares <= 0) return null;
    gross = shares * rawPrice; units = gross / adjPrice;
  }
  var execGross = gross * (1 - slip);
  var fee = otc ? gross * 0.005 : Math.max(execGross * cfg.fee / 1e4, MK.minComm) + execGross * MK.stamp;
  return { units: units, proceeds: Math.max(0, execGross - fee), fee: fee };
}
/* 按固定权重 + 定期再平衡走一条真实资金曲线；rebal=0 表示买入后不再调仓。 */
function weightedCurve(panel, w, rebal, cfg) {
  var n = panel.dates.length, M = panel.syms.length, eq = new Array(n).fill(cfg.cap0), i, m;
  var units = new Array(M).fill(0), cash = cfg.cap0;
  for (i = 1; i < n; i++) {
    var shouldTrade = i === 1 || (rebal > 0 && (i - 1) % rebal === 0);
    if (shouldTrade) {
      var openValue = cash;
      for (m = 0; m < M; m++) openValue += units[m] * panel.syms[m].o[i];
      // 先卖超配资产，再用实际到账现金买入低配资产。
      for (m = 0; m < M; m++) {
        var sy = panel.syms[m], cur = units[m] * sy.o[i], target = openValue * w[m];
        if (cur <= target) continue;
        var so = allocationSellOrder(sy.d, (cur - target) / sy.o[i], sy.ro[i], sy.o[i], cfg);
        if (so) { units[m] = Math.max(0, units[m] - so.units); cash += so.proceeds; }
      }
      for (m = 0; m < M; m++) {
        sy = panel.syms[m]; cur = units[m] * sy.o[i]; target = openValue * w[m];
        var budget = Math.min(cash, Math.max(0, target - cur));
        var bo = allocationBuyOrder(sy.d, budget, sy.ro[i], sy.o[i], cfg);
        if (bo) { units[m] += bo.units; cash -= bo.spent; }
      }
    }
    var closeValue = cash;
    for (m = 0; m < M; m++) closeValue += units[m] * panel.syms[m].c[i];
    eq[i] = closeValue;
  }
  return eq;
}
...
function dcaSim(d, amount, step, cfg) {
  var n = d.c.length, RO = d.ro || d.o;
  var invested = 0, fees = 0, buys = 0, contributions = 0, skipped = 0, units = 0, cash = 0;
  var eq = new Array(n).fill(0), inv = new Array(n).fill(0), i;
  for (i = 1; i < n; i++) {
    if ((i - 1) % step === 0) {
      invested += amount; cash += amount; contributions++;
      var order = allocationBuyOrder(d, cash, RO[i], d.o[i], cfg);
      if (order) { units += order.units; cash -= order.spent; fees += order.fee; buys++; }
      else skipped++;
    }
    eq[i] = cash + units * d.c[i]; inv[i] = invested;
  }
  return { eq: eq, inv: inv, invested: invested, fees: fees, buys: buys, contributions: contributions,
           cash: cash, skipped: skipped,
...
/* ---------- 场内实时快照轮询 ---------- */
var LIVE_QUOTE = { on: false, timer: 0, secid: null, last: '' };
...
function pollQuote(manual) {
  if (!LIVE_QUOTE.secid) return;
  if (!manual && !inTradingHours(isUS({ sym: LIVE_QUOTE.secid }))) { $('btnLive').textContent = '⟳ 实时·休市'; return; }
  fetchQuote(LIVE_QUOTE.secid).then(function (q) {
...
function stopLive() {
  LIVE_QUOTE.on = false; clearInterval(LIVE_QUOTE.timer);
...
$('btnLive').onclick = function () {
  ...
  if (LIVE_QUOTE.on) { stopLive(); log('⟳ 已关闭实时同步', 'sys'); return; }
  LIVE_QUOTE.on = true; LIVE_QUOTE.secid = D.sym;
  ...
  pollQuote(true);
  LIVE_QUOTE.timer = setInterval(pollQuote, 15000);
};
...
function paintSignals() {
  ...
  roll('sOdds', c.odds, 2, uiText('倍', '×'));      bar('bOdds', clamp(c.odds / 5 * 100, 0, 100));
  roll('sPos', c.pos * 100, 1, '%');   bar('bPos', c.pos / 0.8 * 100);
  roll('sEv', c.ev, 2, '%');           bar('bEv', clamp((c.ev + 5) / 20 * 100, 0, 100));
  $('sEvNote').textContent = m
    ? uiText('实测 平均盈利 ', 'Measured avg win ') + fx(m.avgWin * 100, 2) +
      uiText('% / 平均亏损 ', '% / avg loss ') + fx(m.avgLoss * 100, 2) + '% · Kelly f* = ' + fx(c.kelly * 100, 1) + '%'
    : uiText('凯利公式 f* = ', 'Kelly f* = ') + fx(c.kelly * 100, 1) + uiText('% · 建议取一半', '% · half Kelly reference');
  $('sEv').style.color = c.ev >= 0 ? 'var(--up)' : 'var(--down)';
  $('sPosSrc').textContent = c.kelly <= 0
    ? uiText('凯利为负 · 该参数下无正期望，不建议下注', 'Negative Kelly · no positive expectation for these parameters')
    : uiText('半凯利 · 上限 80%', 'Half Kelly · max 80%');
  $('sWinSrc').textContent = manualWin ? uiText('手动输入', 'Manual input')
    : m ? (I18N.isEnglish ? 'Last ' + m.n + ' signals: ' + m.nw + ' wins / ' + m.nl + ' losses'
      : '近' + m.n + '次机会：' + m.nw + ' 胜 / ' + m.nl + ' 负')
    : (D ? uiText('样本不足，暂沿用上次数值', 'Insufficient sample; using prior values') : uiText('来自参数输入', 'From parameters'));
  $('sOddsSrc').textContent = manualOdds ? uiText('手动输入', 'Manual input')
    : m ? uiText('实测 平均盈利 ÷ 平均亏损', 'Measured average win ÷ average loss')
    : (D ? uiText('样本不足，暂沿用上次数值', 'Insufficient sample; using prior values') : uiText('盈亏比 = 目标 ÷ 止损', 'Payoff = target ÷ stop'));
  ...
  $('sRiskLv').textContent = risk > 66 ? uiText('高波动 · 建议减半仓位', 'High volatility · halve reference size')
    : risk > 38 ? uiText('中等波动', 'Medium volatility') : uiText('低波动 · 环境平稳', 'Low volatility · calmer regime');
  ...
  $('sTrendSub').textContent = uiText('20日均线斜率 ', 'MA20 slope ') + (slope >= 0 ? '+' : '') + fx(slope, 2) +
    uiText('% · 价格偏离 ', '% · price distance ') + (above >= 0 ? '+' : '') + fx(above, 2) +
    uiText('% · 量比 ', '% · volume ratio ') + fx(volR, 2) + ' · ' +
    (isFinite(m60) ? (cp > m60 ? uiText('站上60日均线', 'above MA60') : uiText('处于60日均线下方', 'below MA60')) : '—');
...
var STRAT_RUN = { busy: false, id: 0, timer: 0 };
function setStrategyBusy(on) {
  STRAT_RUN.busy = on;
  ['btnAI', 'btnTen', 'btnShort', 'btnRev'].forEach(function (id) { $(id).disabled = on || DATA_OP.busy; });
}
function cancelStrategyRun() {
  STRAT_RUN.id++; clearTimeout(STRAT_RUN.timer); STRAT_RUN.timer = 0; setStrategyBusy(false);
}
function addStrategies(list, dir, label) {
  if (!D) { log('请先点「⛁ 获取数据」载入行情', 'err'); return; }
  if (STRAT_RUN.busy) { log('策略计算正在进行，请等待本批完成', 'sys'); return; }
  if (D.otc) {
  ...
  list = list.filter(function (t) {
    return !CARDS.some(function (c) { return c.st.n === t.n && c.st.dir === dir; });
  });
  if (!list.length) { log('该类策略已全部生成，未新增', 'sys'); return; }
  var cfg = cfgFromForm();
  if (cfg.pos <= 0) log('⚠ 凯利公式结果为负（胜率×赔率不足以覆盖亏损），仓位建议 0%，仅作演示', 'err');
  lbStart(); setStrategyBusy(true);
  var job = ++STRAT_RUN.id, at = 0, added = 0;
  function finish() {
    if (job !== STRAT_RUN.id) return;
    setStrategyBusy(false);
    if (CARDS.length) lastBT = CARDS[CARDS.length - 1].r;
    renderCards(); paintSignals(); lbDone();
    // 批量结果优先展示列表；只自动收起一次，之后尊重用户的手动选择。
    if (!formAutoFolded && CARDS.length >= 5 && !$('stratForm').classList.contains('fold')) {
  ...
  function step() {
    if (job !== STRAT_RUN.id) return;
    if (at >= list.length) { finish(); return; }
    var t = list[at++], c = buildCard(t, dir, cfg);
    CARDS.push(c); added++;
    log('✦ ' + label + '「' + c.st.n + '」回测 ' + (c.r.ret >= 0 ? '+' : '') + fx(c.r.ret, 1) +
        '% · 胜率 ' + fx(c.r.win, 1) + '% · ' + c.r.nt + ' 笔', c.r.ret >= 0 ? 'ok' : 'err');
    STRAT_RUN.timer = setTimeout(step, 20);
  }
  STRAT_RUN.timer = setTimeout(step, 0);
}
...
$('btnClear').onclick = function () { cancelStrategyRun(); CARDS = []; lastBT = null; renderCards(); paintSignals(); log('策略列表已清空', 'sys'); };
...
function loadForScan(id) {
  var it = DB.byId[id];
  var cacheKey = id + '|' + (id.slice(0, 3) === 'OF.' ? 'nav' : adjMode());
  if (SCAN_CACHE[cacheKey]) return Promise.resolve(SCAN_CACHE[cacheKey]);
  var task = id.slice(0, 3) === 'OF.' ? fetchOTCNav(id.slice(3), it.name) : fetchEM(id, it.name);
  return task.then(function (d) {
    // 批量取数不会经过 applyData，必须在这里补回分类；否则 ETF 会被当作股票计印花税。
    d.cat = it.cat || '';
    d.tradable = it.tradable !== false;
    d.__x = indicators(d);
    SCAN_CACHE[cacheKey] = d;
    return d;
  });
}
function runScan(c) {
...
function runAlloc() {
  ...
  Promise.all([worker(), worker()]).then(function () {
    ...
    renderAlloc2(cfg);
    return renderAlloc3(panel, cfg);
  }).catch(function (e) {
    $('alStatus').textContent = '✗ 资产配置计算失败：' + (e && e.message ? e.message : '未知错误');
  }).then(function () {
...
function renderAlloc1(panel, cfg) {
  ...
  var M = panel.syms.length;
  var rows = [], singles = [];
  for (var m = 0; m < M; m++) {
    var w0 = new Array(M).fill(0); w0[m] = 1;
    var s = curveStats(weightedCurve(panel, w0, 0, cfg), 0);
    singles.push(s);
    rows.push({ name: panel.syms[m].name, s: s, hl: false });
  }
  var we = new Array(M).fill(1 / M);
  var eqNoReb = curveStats(weightedCurve(panel, we, 0, cfg), 0);
  var eqReb = curveStats(weightedCurve(panel, we, REBAL_DAYS, cfg), 0);
...
function renderAlloc2(cfg) {
  ...
  function lump(i0) {
    var RO = D.ro || D.o;
    if (!(D.c[i0] > 0 && D.o[i0] > 0 && RO[i0] > 0)) return null;
    var order = allocationBuyOrder(D, dca.invested, RO[i0], D.o[i0], cfg);
    if (!order) return dca.invested;
    return dca.invested - order.spent + order.units * D.c[n - 1];
  }
  var atStart = lump(0);
  ...
  var rows = [
    ['定投（每 ' + STEP + ' 交易日 ' + fx(amount, 0) + ' ' + MKT.unit + ' × ' + dca.contributions + ' 期）', dca.end, dca.invested, true],
  ...
  v.innerHTML = '<b>择时的价值区间</b>：同样一笔钱，买在最倒霉的一天收益 ' + fx(gW, 2) +
    ...
    (dca.skipped ? '<br><span class="dm">有 ' + dca.skipped + ' 期暂时买不起一' + (MKT.lot > 1 ? '手' : '股') +
      '，当期现金已保留并滚入下期；期末剩余现金 ' + fx(dca.cash, 2) + ' ' + MKT.unit + '。</span>' :
      '<br><span class="dm">期末剩余现金 ' + fx(dca.cash, 2) + ' ' + MKT.unit + '，已计入市值。</span>');
}
...
function renderAlloc3(panel, cfg) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var we = new Array(M).fill(1 / M);
    var eqS = curveStats(weightedCurve(panel, we, REBAL_DAYS, cfg), 0);
    var singles = [];
    for (m = 0; m < M; m++) {
      var w1 = new Array(M).fill(0); w1[m] = 1;
      var s1 = curveStats(weightedCurve(panel, w1, 0, cfg), 0);
    ...
    $('alStatus').textContent = '完成 · 公共区间 ' + panel.dates.length + ' 个交易日 · 3000 组配比';
...
function applyData(d, srcTxt) {
  stopLive(); stopPlay(); cancelStrategyRun();
  clearTimeout(DATA_OP.streamTimer); DATA_OP.streamTimer = 0;
  $('mask4').classList.remove('on');
  ...
  var streamData = D, k = Math.max(60, streamData.c.length - 14);
  (function stream() {
    if (streamData !== D) return;
    if (k >= streamData.c.length) { startIdle(); return; }
    logBar(streamData.name, { d: streamData.t[k], o: streamData.o[k], h: streamData.h[k], l: streamData.l[k], c: streamData.c[k], v: streamData.v[k] }, streamData.c[k - 1]);
    k++; DATA_OP.streamTimer = setTimeout(stream, 55);
  })();
}
...
function doFetch() {
  ...
  var requestId = ++DATA_OP.seq;
  if (DATA_OP.controller) DATA_OP.controller.abort();
  DATA_OP.controller = new AbortController(); setDataBusy(true);
  lbStart(); clearInterval(idleTicker); stopPlay(); cancelStrategyRun(); clearTimeout(DATA_OP.streamTimer);
  ...
  var task = otc ? fetchOTCNav(secid.slice(3), name, false, DATA_OP.controller.signal)
                 : fetchEM(secid, name, DATA_OP.controller.signal);
  return task.then(function (d) {
    if (requestId !== DATA_OP.seq) return false;
    d.cat = cat; d.tradable = tradable;
    applyData(d, otc ? '天天基金' : '东方财富');
    lbDone();
    return true;
  }).catch(function (e) {
    if (requestId !== DATA_OP.seq || e.name === 'AbortError') return false;
    log('✗ 在线数据不可用（' + e.message + '），切换本地模拟行情引擎', 'err');
    applyData(simulate(name + '［模拟］', secid), '本地模拟');
    lbDone();
    return false;
  }).finally(function () {
    if (requestId !== DATA_OP.seq) return;
    DATA_OP.controller = null; setDataBusy(false);
  });
}
...
(function bootUniverse() {
  ...
  if (use) {
    loadUniverse(use);
    log('标的库就绪：' + DB.list.length + ' 个标的（A股 / 场内与场外基金 / 美股 / 板块 / 指数），数据日期 ' + DB.ts +
      (DB.staleGroups.length ? '；' + DB.staleGroups.join('、') + ' 仍沿用旧快照' : ''), DB.staleGroups.length ? 'sys' : 'ok');
  } else {
...
setTimeout(function () {
  ...
  log('⚠ 数据以公开日线行情和基金净值为主（非交易所实时逐笔），覆盖 A 股、场内/场外基金、美股、板块与指数；回测结果不构成投资建议', 'err');
}, 500);
...
/* ============================================================
   ...
   ============================================================ */
var STARTUP_HASH = String(location.hash || '');
function normalizeLocalApi(value) {
  value = String(value || '').trim();
  var m = value.match(/^http:\/\/127\.0\.0\.1:(\d{1,5})$/);
  var port = m ? +m[1] : 0;
  return port >= 1 && port <= 65535 ? value : '';
}
var API = (function () {
  var m = STARTUP_HASH.match(/(?:^#|&)qe_api=([^&]+)/);
  var value = '';
  try { value = normalizeLocalApi(m ? decodeURIComponent(m[1]) : ''); } catch (e) { value = ''; }
  if (!value) {
    try { value = normalizeLocalApi(localStorage.getItem('qe_api')); } catch (e) { value = ''; }
  }
  if (!value) value = 'http://127.0.0.1:8770';
  try { localStorage.setItem('qe_api', value); } catch (e) { /* file:// 可能禁用存储 */ }
  return value;
})();
var API_TOKEN = (function () {
  var m = STARTUP_HASH.match(/(?:^#|&)qe_token=([^&]+)/);
  var token = m ? decodeURIComponent(m[1]) : '';
  if (token) {
    try { history.replaceState(null, document.title, location.pathname + location.search); } catch (e) { /* file:// 可拒绝改写 */ }
  }
  return token;
})();
...
function api(path, opt) {
  ...
  if (API_TOKEN) headers['X-QE-Token'] = API_TOKEN;
...
function checkSrv() {
  return api('/api/health', { timeout: 4000 }).then(function (j) {
    if (j.app_id !== 'quant-engine-v10') throw new Error('8770 端口不是量化引擎后端');
    if (!j.authenticated) throw new Error('会话令牌无效，请从量化引擎 EXE 重新打开');
...
var OFFLINE_HTML =
  '<b style="color:var(--up)">后端未连接或会话已失效</b>，「我的持仓」与「市场情绪」需要受保护的本机后端。<br>' +
  '请关闭本页并从 <code>QUANT_ENGINE_v10.exe</code> 重新打开；直接双击 HTML 只能使用离线行情和回测。<br>' +
  '在项目目录下打开命令行，执行：<br>' +
...
function agentSetStatus(ok, model, message) {
  ...
  $('agentState').textContent = ok ? uiText('已连接', 'Connected') : (message || uiText('未连接', 'Disconnected'));
  $('agentModel').textContent = ok ? ('DeepSeek · ' + (model || uiText('量化专家', 'Quant expert'))) :
    uiText('DeepSeek · 后端未连接', 'DeepSeek · Backend offline');
}
function agentWelcome() {
  if (I18N.isEnglish) {
    return '<div class="agentWelcome"><b>I live inside Quant Engine.</b>' +
      'Ask me about this terminal or everyday investing concepts. When needed, I can call the local backend for public quotes, news, sentiment, fund links, holdings or paper-account data. ' +
      'My answers support research and do not predict future returns.</div>';
  }
...
function agentFormat(text) {
  ...
  while (i < lines.length) {
    ...
    if (/^\s*\|.*\|\s*$/.test(line) && i + 1 < lines.length && /^\s*\|[\s:|\-]+\|\s*$/.test(lines[i + 1])) {
      ...
      out.push('<table class="agentMdTable"><thead><tr>' + head.map(function (x) { return '<th>' + agentInline(x) + '</th>'; }).join('') +
        '</tr></thead><tbody>' + rows.map(function (row) { return '<tr>' + head.map(function (_, k) {
          return '<td>' + agentInline(row[k] || '') + '</td>'; }).join('') + '</tr>'; }).join('') + '</tbody></table>');
      continue;
    }
    var head = line.match(/^#{1,3}\s+(.+)$/);
    if (head) { out.push('<div class="agentMdHead">' + agentInline(head[1]) + '</div>'); i++; continue; }
    var numbered = line.match(/^\s*(\d+)\.\s+(.+)$/);
    if (numbered) { out.push('<div class="agentMdItem num" data-n="' + numbered[1] + '">' + agentInline(numbered[2]) + '</div>'); i++; continue; }
    var bullet = line.match(/^\s*[-*]\s+(.+)$/);
    if (bullet) out.push('<div class="agentMdItem">' + agentInline(bullet[1]) + '</div>');
    else if (!line.trim()) out.push('<div class="agentMdGap"></div>');
    else out.push('<div class="agentMdLine">' + agentInline(line) + '</div>');
    i++;
  }
  return out.join('');
}
function agentTrim() { if (AGENT.items.length > AGENT_MAX_MESSAGES) AGENT.items = AGENT.items.slice(-AGENT_MAX_MESSAGES); }
...
function renderAgentMessages() {
  ...
  var html = AGENT.items.map(function (m) {
    ...
    var who = role === 'user' ? uiText('你', 'You') :
      (uiText('量化专家', 'Quant expert') + (m.model ? ' · ' + esc(m.model) : ''));
...
function agentLoad(force) {
  ...
  return api('/api/agent/history?limit=' + AGENT_MAX_MESSAGES, { timeout: 8000 }).then(function (j) {
    AGENT.items = j.items || []; agentTrim(); AGENT.loaded = true;
    var li = j.llm || {};
...
var MOOD_READ_LIMIT = 240;   // 覆盖一轮日常抓取，不把历史积压500条全部塞给模型
...
function renderOverview(o) {
  ...
  if (tot) {
    var seg = function (n, c, t) {
      var pct = n / tot * 100;
      var label = t + n;
      return n ? '<i class="' + (pct < 7.5 ? 'tiny' : '') + '" aria-label="' + label + '，占 ' + pct.toFixed(1) + '%" title="' + label +
...
$('moodRead').onclick = function () {
  ...
  var runRead = api('/api/news/read/start?limit=' + MOOD_READ_LIMIT, { method: 'POST', timeout: 30000 }).then(poll).then(function (j) {
...
$('paperGenerate').onclick = function () {
  ...
  busy(b, '生成中…', api('/api/paper/accounts/' + PAPER.current.id + '/signals', { method: 'POST', timeout: 300000 })
    .then(function (j) { PAPER.current = j.detail; renderPaper(j.detail);
      log('⚙ 模拟盘生成 ' + j.signals + ' 个信号、' + j.proposals + ' 笔拟交易；信号日不成交，' +
        (PAPER.current && PAPER.current.mode === 'auto' ? '模拟自动模式将等待下一有效交易日' : '等待人工确认且最早下一交易日执行'), 'ok');
...
function renderWealth(j) {
  ...
  var refreshErrors = j.refresh_errors || [];
  ...
  var refreshWarning = refreshErrors.length ? '<div class="simEmpty" style="color:var(--gold);text-align:left">⚠ ' +
    refreshErrors.length + ' 个账户的行情未完整对齐，本次未写入新的财富曲线，下表保留上一个有效交易日数据。</div>' : '';
  $('wealthTable').innerHTML = refreshWarning + (items.length ? '<table class="simTable"><thead><tr><th>虚拟账户</th><th>模式</th><th>初始资金</th><th>总资产</th>' +
...
/* ---------- 实盘数据只读 / 不可执行订单意图 ---------- */
var LIVE_READONLY = { snapshot: null, intents: [] };
...
function loadLiveData() {
  ...
  return api('/api/live/read-only/status?account_alias=' + encodeURIComponent(liveAlias()), { timeout: 30000 })
    .then(function (j) {
      ...
      LIVE_READONLY.snapshot = j.snapshot || null; LIVE_READONLY.intents = j.intents || [];
      renderLiveSnapshot(LIVE_READONLY.snapshot); renderLiveIntents(LIVE_READONLY.intents);
      return j;
    }).catch(function (e) {
...
function renderLiveIntents(rows) {
  LIVE_READONLY.intents = rows || [];
  $('liveIntentState').innerHTML = '<span class="liveLock">执行权限关闭</span> · ' + LIVE_READONLY.intents.length + ' 条订单意图，仅可复核和导出';
  $('liveIntents').innerHTML = LIVE_READONLY.intents.length ? '<table class="simTable"><thead><tr><th>意图编号</th><th>方向</th><th>标的</th><th>数量</th><th>参考价</th><th>最大偏离</th><th>风控状态</th><th>到期时间</th></tr></thead><tbody>' +
    LIVE_READONLY.intents.map(function (x) { return '<tr><td class="dm">' + esc(x.intent_id) + '</td><td class="' + (x.side === 'buy' ? 'buy' : 'sell') + '">' +
