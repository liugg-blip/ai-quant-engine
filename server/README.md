...
Windows 可以直接双击 `run.bat`，它会建虚拟环境、装依赖、启动服务。通过 EXE 启动时，
启动器会在 `8770-8780` 中自动复用兼容后端或选择空闲端口；端口冲突不会再阻塞进入按钮，
后端连接期间可以先进入基础终端。

情绪打分、每日建议和驻留量化专家需要大模型。优先把 `.env.example` 复制成 `.env` 并填 `LLM_API_KEY`：
...
为兼容旧版本提示，如果 `.env` 不存在但 `.env.example` 已填写，首次启动会自动复制为正式 `.env`；以后以 `.env` 为准，修改后需重启后端。

直接打开 HTML 时，前端默认连接 `http://127.0.0.1:8770`。通过 EXE 打开时，启动器会把实际
选中的本机端口传给网页。出于会话令牌安全考虑，网页只接受 `http://127.0.0.1:<端口>`，
不会连接 localStorage 或启动参数中设置的远程地址。
...
只保留手动刷新）。默认每轮最多阅读最新 240 条，8 条一批、最多 10 批并行；已打分新闻不会重复调用模型。

## 接口

| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/api/health` | 无令牌时只返回身份、版本和授权状态；启动器令牌通过后才返回模型配置、库内计数与抓取状态 |
...
| POST | `/api/paper/accounts/{id}/signals` | 生成复合信号和拟交易清单；模拟自动模式也要等下一有效交易日开盘后才写入假钱成交 |
...
除 `/api/health` 的最小身份信息外，全部本机接口都要求 EXE 启动时生成的随机 `X-QE-Token`；允许的跨域来源仅限本机页面。令牌只写入 `data/session.token` 并通过 URL 片段交给前端，前端读取后立即从地址栏移除。
...
模拟自动模式也只把结果写入上述 SQLite 表。信号按当日收盘信息生成，成交必须使用下一有效交易日开盘；下一根行情不存在时保持待处理，避免同日收盘前视偏差。`paper_lots` 按买入日期保存剩余批次，执行卖出时只读取
