...
async def src_wallstreetcn(c: httpx.AsyncClient) -> list[dict]:
    ...
    for it in items:
        ...
        if not text:
            continue
        raw_time = it.get("display_time") or it.get("created_at")
        pub, day = _from_ts(raw_time) if raw_time else _now()
...
UNAVAILABLE_SOURCES = {
    "财联社": "公开接口要求官方签名或授权，当前未接入，改用金十数据与交易所公告补充",
    "Reuters": "官方公开 RSS 已停止服务，当前未接入，改用 CNBC、MarketWatch 与 SeekingAlpha 补充",
}
...
async def crawl_all() -> dict:
    ...
    all_items: list[dict] = []
    per_source = {name: {"count": 0, "error": reason, "available": False}
                  for name, reason in UNAVAILABLE_SOURCES.items()}
    for name, items, err in results:
        per_source[name] = {"count": len(items), "error": err, "available": not bool(err)}
