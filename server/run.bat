...
echo [2/2] 启动后端 http://127.0.0.1:8770
.venv\Scripts\python.exe -m uvicorn main:app --host 127.0.0.1 --port 8770
exit /b %errorlevel%

:global
echo [1/1] 使用本机 Python 启动后端 http://127.0.0.1:8770
python -m uvicorn main:app --host 127.0.0.1 --port 8770
exit /b %errorlevel%
